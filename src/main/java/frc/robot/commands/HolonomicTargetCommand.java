...
public class HolonomicTargetCommand extends Command {
  ...
  public void execute() {
    ...
    if (hasTarget) {
      ...
      // rotation Y - pitch - -90-degrees is flat on floor - rotation is positive as
      // tilted toward camera
      // - visible targets in range [-90, 90]
      // rotation X - roll - 0-degrees is straight upward (or straight down) -
      // clockwise rotation is positive
      // - seems to give same results if the target is upside down (maybe need to
      // research this one)
      // - visible targets are in range [-180, 180]
      // rotation Z - yaw - -90-degrees is perpendicular to the camera, rotated with
      // the right side away from camera (not visible)
      // - 180-degrees is straight on with the camera
      // - from the camera's perspective, rotation the left side of the target closer
      // is positive
      ...
      //Robot.distanceToTarget = distanceFromTarget;
