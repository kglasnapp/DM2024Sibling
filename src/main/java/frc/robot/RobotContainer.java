...
public class RobotContainer {
  ...
  public final GrabberSubsystem grabberSubsystem = (testMode) ? null : new GrabberSubsystem();
  ...
  // controller.pov(0).onTrue(new SetShooterAngleCommand(shooterSubsystem, false,
  // 0)); // Retrack
  // pov(90) set shooter to minimum angle
  // controller.pov(180).onTrue(new SetShooterAngleCommand(shooterSubsystem,
  // false, -30));
  // pov(270) set shooter to amp shot
  // controller.pov(270).onTrue(new SetShooterAngleCommand(shooterSubsystem, true,
  // -4)); // Up extended
  // controller.pov(90).onTrue(Autonomous.grabNoteCommand(this));
  // controller.pov(90).onTrue(new StraightPathCommand(drivetrainSubsystem,
  ...
  // grabberSubsystem,
  // limeLightPoseSubsystem,
  // drivetrainSubsystem).alongWith(new
  // SpeakerAlligningCommand(limeLightPoseSubsystem, drivetrainSubsystem)));
  // controller.b().onTrue(
  ...
  public void configureDriverController(CommandXboxController controller) {
    ...
    driveController.y().onTrue(
        new ShootCommand(shooterSubsystem2, indexerSubsystem));
